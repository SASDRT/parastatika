      ...
      <div style={C.content}>
                    ...
                    <label style={{ ...C.btnGhost, display: 'inline-flex', alignItems: 'center', gap: 8, padding: '13px 22px', fontSize: 14, borderRadius: 9 }}>
                      ...
                      <input type="file" accept="image/*,.pdf" multiple style={{ display: 'none' }} onChange={async e => {
                        ...
                        try {
                          ...
                          const imgContent = dataUrls.map((d, i) => ({ type: 'image', source: { type: 'base64', media_type: files[i].type || 'image/jpeg', data: d.split(',')[1] } }))
                          const res = await fetch('/api/ocr', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ multiContent: imgContent }) })
                          const json = await res.json()
                          if (json.success) setEditForm(json.data)
        ...
        {(tab === 2 || tab === 3) && (
          <InvoiceList
            ...
            tab={tab} copyInvoice={copyInvoice} userRole={userRole} loadInvoices={loadInvoices} notify={notify}
...
function KartelesTab({ invoices, payments, byCounterparty, fmt, fmtDate, year, month, monthsFull }) {
  ...
  const printKartela = () => {
    ...
    <div class="tr"><span>Χρεώσεις:</span><span class="red">${selected.invoices.filter(i=>{const t=(i.invoice_type||'').toLowerCase(),n=(i.number||i.series||'').toLowerCase();return !t.includes('πιστωτικό')&&!t.includes('πιστωτικο')&&!t.includes('επιστροφή')&&!t.includes('επιστροφη')&&!t.includes('credit')&&!n.startsWith('πισ')&&!n.startsWith('pis')}).reduce((s,i)=>s+(i.total||0),0).toFixed(2)}€</span></div>
    <div class="tr"><span>Πιστώσεις:</span><span class="grn">${(()=>{const cn=selected.invoices.filter(i=>{const t=(i.invoice_type||'').toLowerCase(),n=(i.number||i.series||'').toLowerCase();return t.includes('πιστωτικό')||t.includes('πιστωτικο')||t.includes('επιστροφή')||t.includes('επιστροφη')||t.includes('credit')||n.startsWith('πισ')||n.startsWith('pis')}).reduce((s,i)=>s+(i.total||0),0);const pType=cpType==='expense'?'payment':'receipt';const pm=(payments||[]).filter(p=>(p.afm===selected.afm||(p.counterparty||'').toLowerCase()===selected.name.toLowerCase())&&p.type===pType).reduce((s,p)=>s+(p.amount||0),0);return (cn+pm).toFixed(2)})()}€</span></div>
    <div class="tr grand"><span>ΥΠΟΛΟΙΠΟ:</span><span>${(()=>{const dr=selected.invoices.filter(i=>{const t=(i.invoice_type||'').toLowerCase(),n=(i.number||i.series||'').toLowerCase();return !t.includes('πιστωτικό')&&!t.includes('πιστωτικο')&&!t.includes('επιστροφή')&&!t.includes('επιστροφη')&&!t.includes('credit')&&!n.startsWith('πισ')&&!n.startsWith('pis')}).reduce((s,i)=>s+(i.total||0),0);const cr=selected.invoices.filter(i=>{const t=(i.invoice_type||'').toLowerCase(),n=(i.number||i.series||'').toLowerCase();return t.includes('πιστωτικό')||t.includes('πιστωτικο')||t.includes('επιστροφή')||t.includes('επιστροφη')||t.includes('credit')||n.startsWith('πισ')||n.startsWith('pis')}).reduce((s,i)=>s+(i.total||0),0);const pType=cpType==='expense'?'payment':'receipt';const pm=(payments||[]).filter(p=>(p.afm===selected.afm||(p.counterparty||'').toLowerCase()===selected.name.toLowerCase())&&p.type===pType).reduce((s,p)=>s+(p.amount||0),0);return (dr-cr-pm).toFixed(2)})()}€</span></div>
    </div></div>
...
═══════════════════════════════════════ */
function InvoiceList({ list, color, title, searchQ, setSearchQ, filtered, expandedId, setExpandedId, deleteInvoice, setTab, setEditForm, fmt, fmtDate, loading, tab, copyInvoice, userRole, generalExpenses = [], loadInvoices, notify }) {
  const [flagged, setFlagged] = useState({}) // local state για άμεση εμφάνιση
